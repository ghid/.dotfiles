...
--  Custom keymaps
vim.keymap.set('n', '<leader>QQ', '<cmd>qa!<CR>', { desc = 'Cose immediately' })
    ...
    config = function()
      ...
      lint.linters.shellcheck = {
        ...
        parser = function(output)
          ...
          -- Parsen der shellcheck-Ausgabe
          for _, line in ipairs(vim.split(output, '\n')) do
            local filename, lnum, col, severity, message = line:match '(.+):(%d+):(%d+):%s*(%w+):%s*(.+)'

            if filename and lnum and col and severity and message then
